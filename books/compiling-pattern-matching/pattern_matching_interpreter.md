...
#### Rustのenumについて
`enum Name { /* definition */ }` は代数的データ型を定義します。ヴァリアント（Rustにおけるコンストラクタ）の定義方法が3種類あります。引数なし、`()` 囲みのカンマ区切り（例えば `Expr::Tuple`）、 `{}` 囲みの `名前: 型`（例えば `Expr::Inject`）です。SMLと違ってヴァリアントの引数は複数とれます。
